# -*- coding: utf-8 -*-
"""
Created on Sat Dec 12 11:59:38 2020
...
class TwoNN():
	...
	def _fit(self,X):
		...
		if self.method=='MaxLikelihood':
			...
			par,cov= np.polyfit(np.log(mu[:Neff]),-np.log(1-F[:Neff]),1,cov=True)
			DimEstimate = par[0]

		return  DimEstimate,mu,cov


	def fit(self, X):
		DimEstimate, mu, cov = self._fit(X)
		self.DimEstimate_ = DimEstimate
		self.mu_ = mu; self.cov_ =cov
		N,d = np.shape(X)
		...
		if self.block_analysis==True: 
			...
			for nblocks in range(1,maxnblocks+1):
				...
				for b in range(nblocks):
					Y=X[idx[b*int(blocksize):(b+1)*int(blocksize)],:]
					bde,mu,cov=self._fit(Y) 					
	...
	def ShowLinearFit(self):
		...
		plt.xlabel("$\log(\mu)$")
		plt.ylabel("$\log(1-F)$")
		plt.title("ID = %(DimEstimate)2.2f" % {"DimEstimate": DimEstimate})
